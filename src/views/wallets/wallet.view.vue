  ...
  watch: {
    ...
    wallet: {
      handler(){
        ...
        let total = this.parsedInvoices.reduce((acc, i) => acc + i.amount, 0)
        this.parsedInvoices.push({
          ...
          descontado: this.parsedInvoices.reduce((acc, i) => acc + i.descontado, 0),
          neto: this.parsedInvoices.reduce((acc, i) => acc + i.neto, 0),
          recibido: this.parsedInvoices.reduce((acc, i) => acc + i.recibido, 0),
          entregado: this.parsedInvoices.reduce((acc, i) => acc + i.entregado, 0),
          TCEA: this.parsedInvoices.reduce((acc, i) => acc + i.TCEA * i.amount, 0) / total,
        })
      },
      deep: true,
      immediate: true
  ...
  <div class="main__container flex col col-top-center max-height max-width padding-2 gap-1" v-else-if="wallet">
    ...
    <DataTable :value="parsedInvoices" :paginator="true" :rows="5" :rowsPerPageOptions="[5,10,20]"
      ...
      <Column header="Emisión" field="emission">
        <template #body="slotProps">
          <div v-if="slotProps.data.emission">
      ...
      <Column header="Vencimiento" field="expiration">
        <template #body="slotProps">
          <div v-if="slotProps.data.expiration">
      ...
      <Column>
        ...
        <template #body="slotProps">
          <div >
            {{currency.symbol}}{{(slotProps.data.descontado / currency.rate).toFixed(2)}}
          </div>
        </template>
      </Column>
      <Column header="Valor Neto">
        <template #body="slotProps">
          <div >
            {{currency.symbol}}{{(slotProps.data.neto/ currency.rate).toFixed(2)}}
          </div>
        </template>
      </Column>
      <Column header="Valor Recibido">
        <template #body="slotProps">
          <div >
            {{currency.symbol}}{{(slotProps.data.recibido / currency.rate).toFixed(2)}}
          </div>
        </template>
      </Column>
      <Column header="Valor Entregado">
        <template #body="slotProps">
          <div >
            {{currency.symbol}}{{(slotProps.data.entregado / currency.rate).toFixed(2)}}
          </div>
        </template>
      </Column>
      ...
      <Column header="TCEA">
        <template #body="slotProps">
          <div >
            {{ (slotProps.data.TCEA * 100).toFixed(2) }}%
          </div>
        </template>
      </Column>
      <Column>
        ...
        <template #body="slotProps">
          <div v-if="!slotProps.data.resume">
