...
import {parseDate, calcTCEA, calcInteresDescontado} from "@/services/functions.js";
...
<template>
  <createInvoice :visible="dialogs.newInvoice"
  ...
  <div class="main__container flex col col-top-center max-height max-width padding-2 gap-1" v-else-if="wallet">
    ...
    <DataTable :value="wallet.invoices" :paginator="true" :rows="5" :rowsPerPageOptions="[5,10,20]"
      ...
      <Column header="Interés Descontado">
        <template #body="slotProps">
          <div>{{calcInteresDescontado({
                fechaDescuento: wallet.discount.date,
                fechaFinal: slotProps.data.expiration,
                porcentajeTasa: wallet.discount.percentage,
                tipoTasa: wallet.discount.type,
                periodoTasa: wallet.discount.period,
              })}}</div>
          <!--
      ...
      <Column header="TCEA">
        <template #body="slotProps">
          <div>{{ calcTCEA({
            monto: slotProps.data.amount,
            gastoInicial: wallet.startingBills,
            gastoFinal: wallet.endingBills,
            fechaDescuento: wallet.discount.date,
            fechaFinal: slotProps.data.expiration,
            porcentajeTasa: wallet.discount.percentage,
            tipoTasa: wallet.discount.type,
