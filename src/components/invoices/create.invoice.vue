<script>
import { formatDateToOnlyDate } from '@/utils/date'
  ...
  methods: {
    ...
    save() {
      ...
      if (!this.invoice.name)
        return this.sendErrorMsg('Por favor, complete el nombre de la factura.')
      if (!this.invoice.client.name)
        return this.sendErrorMsg('Por favor, complete el nombre del cliente.')
      if (!this.invoice.amount)
        return this.sendErrorMsg('Por favor, complete el monto de la factura.')
      if (!this.invoice.emission)
        return this.sendErrorMsg(
          'Por favor, complete la fecha de emisión de la factura.'
        )
      if (!this.invoice.expiration)
        return this.sendErrorMsg(
          'Por favor, complete la fecha de vencimiento de la factura.'
        )
      if (this.invoice.emission > this.invoice.expiration)
        return this.sendErrorMsg(
  ...
  watch: {
    emissionDate: {
      handler: function (val) {
        this.invoice.emission = val.getTime()
      },
      deep: true,
    },
    expirationDate: {
      handler: function (val) {
        this.invoice.expiration = val.getTime()
      },
      deep: true,
    },
  },
