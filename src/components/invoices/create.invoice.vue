  ...
  watch: {
    ...
    money: {
      handler: function (val) {
        this.invoice.amount = val * this.currency.rate
      },
      deep: true
    }
  }
  ...
  <Dialog :visible="visible" modal header="Crear Nueva Factura" :style="{ width: '20rem' }" >
    ...
    <div class="flex col gap-0-5 center">
      ...
      <FloatLabel>
        ...
        <InputNumber id="amount" class="flex-auto" mode="currency" :currency="currency.small" v-model="money"/>
      </FloatLabel>
      <FloatLabel>
        <label for="emission">Fecha de Emisión</label>
        <Calendar id="emission" class="flex-auto" v-model="emissionDate" dateFormat="dd/mm/yy"/>
      </FloatLabel>
      <FloatLabel>
        <label for="expiration">Fecha de Vencimiento</label>
        <Calendar id="expiration" class="flex-auto" v-model="expirationDate" dateFormat="dd/mm/yy"/>
      </FloatLabel>
