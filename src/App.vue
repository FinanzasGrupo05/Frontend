  ...
  methods: {
    ...
    async unlinkInvoice(invoice) {
      try {
        ...
        if (this.wallet)
          this.wallet.invoices = this.wallet.invoices.filter(
            (i) => i.id !== invoice.id
          )
    ...
    async deleteInvoice(invoice) {
      try {
        ...
        if (invoice.walletId) {
          ...
          if (wallet) {
            wallet.invoices = wallet.invoices.filter((i) => i !== invoice)
          }
        }
        //await 1 seconds
        await new Promise((resolve) =>
    ...
    async createWallet(wallet) {
      try {
        ...
        wallet.id = this.wallets.length
          ? this.wallets[this.wallets.length - 1].id + 1
          : 1
