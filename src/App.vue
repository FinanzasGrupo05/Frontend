...
import {keys, sexes, calendarLabels} from "@/services/values.js";
...
export default {
  ...
  methods: {
    ...
    async unlinkInvoice(invoice){
      try {
        ...
        this.invoices.find(i => i.id === invoice.id).walletId = null
    ...
    async linkInvoice(invoice) {
      try {
        this.processing = 'Vinculando Factura...'

        this.wallet = this.wallets.find(w => w.id === this.wallet.id)
        if (this.wallet) this.wallet.invoices.push(invoice)
        this.invoices.find(i => i.id === invoice.id).walletId = this.wallet.id
  ...
  created() {
    //this.invoices.forEach((invoice, index) => { invoice.id = index })
    //setLocalStorageItem(keys.invoices, this.invoices)

    //Traducir el calendario al español:
    for(let prop in calendarLabels) this.$primevue.config.locale[prop] = calendarLabels[prop];
...
<template>
  ...
  <header :class="user?'appear':'vanish'">
    <div class="flex row row-justify-center padding-0-5" v-if="user">
      <img :src="dark?'/logo-dark.png':'/logo-light.png'" alt="Logo" class="header__logo"/>
      <div class="flex row center padding-1">
        <div>
          {{
            sexes.find(s => s.value === user.sex)?
            sexes.find(s => s.value === user.sex).welcome:
            'Hola'
          }},
          {{ user.name }}
        </div>
